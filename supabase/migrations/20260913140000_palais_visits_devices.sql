-- Visitor book, part 2: what people visit on — phone, tablet or desktop, and
-- which system and browser (e.g. iPhone · Safari). Run after
-- 20260913130000_palais_visits.sql. Safe to run more than once.

alter table public.palais_visits add column if not exists os text;
alter table public.palais_visits add column if not exists browser text;

drop function if exists public.palais_record_visit(text, text, text, text, text, text, text, numeric, numeric, text, text);
...
begin
  ...
  insert into public.palais_visits
    (visitor, session, page, referrer, city, region, country, country_code, lat, lon, timezone, device, os, browser)
  values (
...
begin
  ...
  return jsonb_build_object(
    ...
    'recent', coalesce((
      select jsonb_agg(jsonb_build_object('at', created_at, 'city', city, 'region', region, 'country', country, 'code', country_code, 'page', page, 'referrer', referrer, 'device', device, 'os', os, 'browser', browser, 'visitor', left(visitor, 6)) order by created_at desc)
...
revoke all on function public.palais_record_visit(text, text, text, text, text, text, text, numeric, numeric, text, text, text, text) from public;
grant execute on function public.palais_record_visit(text, text, text, text, text, text, text, numeric, numeric, text, text, text, text) to anon, authenticated;
