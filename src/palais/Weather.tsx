...
/**
 * Snow in winter, falling leaves in autumn.
 *
 * The weather follows the photographs rather than a clock of its own: a few
 * times a second it reads how far the winter and autumn photographs have faded
...
const LAYERS: Record<Layer, { snow: number; leaves: number; scale: [number, number]; blur: number }> = {
  back: { snow: 260, leaves: 55, scale: [0.6, 1.3], blur: 0 },
  front: { snow: 40, leaves: 10, scale: [1.8, 3], blur: 1.5 },
};
...
export function Weather({ layer }: { layer: Layer }) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const want = { snow: Math.round(cfg.snow * winter), leaf: Math.round(cfg.leaves * autumn) };
      const have = { snow: 0, leaf: 0 };
      ...
      for (const kind of ["snow", "leaf"] as Kind[]) {
